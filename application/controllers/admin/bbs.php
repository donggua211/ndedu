...
class Bbs extends Controller {
	...
	*/
	function add()
	{
		...
		{
			//必填信息.
			$new_bbs['bbs_title'] = $this->input->post('bbs_title');
			$new_bbs['bbs_content'] = $this->input->post('bbs_content');
			$new_bbs['staff_id'] = $this->staff_info['staff_id'];
			
			if(empty($new_bbs['bbs_title']) || empty($new_bbs['bbs_content']))
			{
				$notify = '请填写完整的员工信息';
				$this->_load_bbs_add_view($notify, $new_bbs);
			}
			else
			{
				//add into DB
				if($this->CRM_Bbs_model->add($new_bbs))
				{
					show_result_page('评论已经添加成功! ', 'admin/bbs');
				}
				else
				{
					$notify = '评论添加失败, 请重试.';
	...
	function _load_bbs_add_view($notify = '', $bbs = array())
	{
		$data['header']['meta_title'] = '添加评论 - 内部评论';
		$data['main']['notification'] = $notify;
		$data['main']['bbs'] = $bbs;
		_load_viewer($this->staff_info['group_id'], 'bbs_add', $data);
	}
